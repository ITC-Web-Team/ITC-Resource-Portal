...
export default function Navbar() {
    ...
    return (
        <div className="sticky top-0 z-20 px-4 pt-4">
            <header className="mx-auto grid max-w-6xl grid-cols-[auto_1fr_auto] items-center gap-4 rounded-2xl border border-white/10 bg-[#111116]/90 px-5 py-3 shadow-xl shadow-black/40 backdrop-blur-md">

                <Link
                    to="/"
                    className="col-start-1 transition-opacity hover:opacity-80"
                >
                    <BrandLockup />
                </Link>

                <nav className="col-start-2 hidden items-center justify-self-center gap-1 rounded-full border border-white/10 bg-white/5 p-1 text-xs font-bold tracking-wide text-gray-300 md:flex">
                    {navLinks.map((link) => (
                        <a
                            key={link.label}
                            href={link.href}
