...
import { fetchProfile, fetchMentor, fetchMyProjects } from "../js/UserApi";
...
export default function MyProfilePage() {
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      ...
      setError(null);
      try {
        const [profileData, mentorData, projectsData] = await Promise.all([
          fetchProfile(),
          fetchMentor(),
          fetchMyProjects(),
        ]);
    ...
    <div className="min-h-screen w-full bg-[#0a0a0f] text-white relative overflow-hidden">
      ...
      <div className="relative mx-auto flex w-full max-w-[1440px] flex-col">
        <header className="grid grid-cols-[auto_1fr_auto] items-center gap-6 px-6 py-6 md:px-16">
          ...
          <span className="justify-self-end rounded-md border-2 border-[#FD6E59] px-5 py-2.5 text-xs font-extrabold tracking-wide text-white">
            MY PROFILE
          </span>
        </header>
        ...
        <section className="px-6 pb-10 pt-6 text-center md:px-16">
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-gray-400 sm:text-base">
            Your ideas are taking shape. Track your projects, approvals, funding
                ...
                <div className="min-w-[200px] flex-1">
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-3">
                    ...
                    <span className="flex items-center gap-1.5 rounded-full bg-green-500/15 px-3 py-1.5 text-xs font-semibold text-green-400">
                      <CheckCircle2 size={12} /> SSO verified
                    </span>
                  </div>
                </div>
              </div>
        ...
        <section className="px-6 pb-10 md:px-16">
          <div className="rounded-2xl border border-white/10 bg-[#111116]/80 p-8">
            ...
            {loading ? (
              <p className="text-sm text-gray-500">Loading mentor info…</p>
            ) : (
              <div className="flex flex-wrap items-center gap-6">
                ...
                <div className="min-w-[200px] flex-1">
                  ...
                  <p className="mt-1 text-sm text-gray-400">{mentor.role}</p>
                  <div className="mt-3 flex flex-wrap items-center gap-3">
                    <span className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-gray-300">
                      <Mail size={12} /> {mentor.email}
                    </span>
                    <span className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-gray-300">
                      <Phone size={12} /> {mentor.phone}
                    </span>
                  </div>
                </div>
              </div>
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-400">
                        {project.category} · Deadline {project.deadline}
                      </p>
                    </div>
                  ...
                  <div className="mt-6 flex flex-wrap gap-10">
                    <div>
                      <p className="text-lg font-extrabold text-white">{project.allotted}</p>
                      <p className="mt-1 text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                        Allotted
                      </p>
                    </div>
                    <div>
                      <p className="text-lg font-extrabold text-white">{project.timeline}</p>
                      <p className="mt-1 text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                        Timeline
                      </p>
                    </div>
                    <div>
                      <p className="text-lg font-extrabold text-white">{project.period}</p>
                      <p className="mt-1 text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                        Period
                      </p>
                    </div>
